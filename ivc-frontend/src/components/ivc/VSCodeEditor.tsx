...
const VSCodeEditor = ({ files, onClose, onSave }: VSCodeEditorProps) => {
    const [activeFile, setActiveFile] = useState<FileStatus | null>(
        files[0] || null
    );
    ...
    const handleContentChange = (content: string) => {
        if (activeFile) {
            setFileContents(
                (prev) => new Map(prev.set(activeFile.name, content))
            );
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-slate-900 z-50 flex flex-col !my-4 mx-4 rounded-lg overflow-hidden">
            {/* Title Bar */}
            <div className="h-8 bg-slate-800 flex items-center justify-between px-2 border-b border-slate-700">
                <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-slate-300 ml-4">
                        Impact Vibe Coder - VS Code Editor
                    </span>
                </div>
                <button
                    onClick={onClose}
            ...
            <div className="h-8 bg-slate-800 flex items-center px-4 text-sm text-slate-300 border-b border-slate-700">
                <span className="hover:bg-slate-700 px-2 py-1 rounded cursor-pointer">
                    File
                </span>
                <span className="hover:bg-slate-700 px-2 py-1 rounded cursor-pointer">
                    Edit
                        ...
                        {activeFile ? (
                            <>
                                ...
                                <div className="flex-1 relative">
                                    <textarea
                                        value={fileContents.get(
                                            activeFile.name
                                        )}
                                        onChange={(e) =>
                                            handleContentChange(e.target.value)
                                        }
                                        className="w-full h-full p-4 pl-16 bg-slate-900 text-slate-200 font-mono text-sm resize-none focus:outline-none"
                                        spellCheck={false}
                                    ...
                                    <div className="absolute left-0 top-0 w-12 h-full bg-slate-800 border-r border-slate-700 p-4">
                                        {(
                                            fileContents.get(activeFile.name) ||
                                            ""
                                        )
                                            .split("\n")
                                            .map((_, index) => (
                                                <div
                                                    key={index}
                                                    className="text-xs text-slate-500 leading-5 text-right"
                                                >
                                                    {index + 1}
                                                </div>
                                            ))}
