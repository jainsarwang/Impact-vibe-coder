    ...
    const handleAddTokens = async () => {
        ...
        if (!isActive) {
            toast({
                ...
                description:
                    "You can't add tokens to an inactive organization. Please activate the organization first.",
                variant: "destructive",
            });
            return;
        }
        ...
        try {
            ...
            toast({
                ...
                description: `${tokensToAdd.toLocaleString()} tokens have been added to ${
                    organization.organization_name
                }`,
            });
                ...
                <div className="space-y-3">
                    <div>
                        ...
                        <p className="text-xs text-gray-600">
                            Current tokens:{" "}
                            {organization?.total_tokens?.toLocaleString()}
                        </p>
                        <p className="text-xs text-gray-600">
                            Available tokens:{" "}
                            {organization?.tokens_remaining?.toLocaleString()}
                        </p>
                        <p className="text-xs text-gray-600">
                        ...
                        <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex items-start space-x-2">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-red-700">
                                    You can't add tokens to an inactive
                                    organization. Please activate the
                                    organization first, then try adding tokens.
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    {isActive && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-2">
                            <p className="text-xs text-blue-700">
                                These tokens will be added to the organization's
                                total token pool.
                            </p>
                        </div>
                    )}
