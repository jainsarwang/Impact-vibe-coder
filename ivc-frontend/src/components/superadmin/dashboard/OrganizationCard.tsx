...
import { Card, CardContent, CardHeader } from '@/components/superadmin/ui/card';
import { Button } from '@/components/superadmin/ui/button';
import { Badge } from '@/components/superadmin/ui/badge';
import { Switch } from '@/components/superadmin/ui/switch';
import { Coins, Eye, Users, Calendar, Building2, Activity, Clock } from 'lucide-react';
...
const OrganizationCard = ({ org, onViewOrganization, onStatusUpdated }: OrganizationCardProps) => {
  ...
  const adminCount = org.users?.filter(user => user.role_id && !user.role_id.includes('user')).length || 0;

  const handleStatusToggle = async () => {
    setIsUpdatingStatus(true);
    try {
      const newStatus = !isActive;
      console.log('Toggling status for organization:', org.organization_name, 'from', isActive, 'to', newStatus);
  ...
  const usagePercentage = org.total_tokens > 0 ? (tokensUsed / org.total_tokens) * 100 : 0;
  ...
  return (
    <Card className="group hover:shadow-lg transition-all duration-200 border border-gray-200 bg-white overflow-hidden hover:-translate-y-1">
      {/* Status Indicator */}
      <div className={`h-1 ${isActive ? 'bg-gradient-to-r from-emerald-500 to-emerald-600' : 'bg-gradient-to-r from-gray-400 to-gray-500'}`}></div>
      
      <CardHeader className="pb-1 pt-2 px-2">
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-2 min-w-0 flex-1">
            <div className={`w-5 h-5 ${isActive ? 'bg-gradient-to-br from-blue-500 to-indigo-600' : 'bg-gradient-to-br from-gray-400 to-gray-500'} rounded-md flex items-center justify-center shadow-sm`}>
      ...
      <CardContent className="px-2 pb-2">
        ...
        <div className="grid grid-cols-2 gap-1.5 mb-2">
          <div className="text-center p-1.5 bg-gradient-to-br from-blue-50 to-blue-100 rounded-md border border-blue-200">
            ...
            <p className="text-xs text-blue-700 font-medium mb-0.5">Users</p>
            <p className="text-xs font-bold text-blue-900">{org.user_count?.toLocaleString() || 0}</p>
          </div>
          ...
          <div className="text-center p-1.5 bg-gradient-to-br from-yellow-50 to-amber-100 rounded-md border border-yellow-200">
            ...
            <p className="text-xs text-amber-700 font-medium mb-0.5">Tokens</p>
            <p className="text-xs font-bold text-amber-900">{org.total_tokens?.toLocaleString() || 0}</p>
          </div>
        </div>
        ...
        <div className="mb-2">
          ...
          <div className="w-full bg-gray-200 rounded-full h-1">
            <div 
              className={`${isActive ? 'bg-gradient-to-r from-blue-500 to-indigo-600' : 'bg-gray-400'} h-1 rounded-full transition-all duration-300`}
              style={{ width: `${Math.min(usagePercentage, 100)}%` }}
            ></div>
          </div>
          <div className="flex justify-between text-xs text-gray-500 mt-0.5">
            <span>{tokensUsed.toLocaleString()}</span>
            <span>{(org.tokens_remaining || 0).toLocaleString()}</span>
          </div>
        </div>
        ...
        <Button
          ...
          className={`w-full ${isActive ? 'bg-gradient-to-r from-blue-600 to-indigo-700 hover:from-blue-700 hover:to-indigo-800' : 'bg-gray-500 hover:bg-gray-600'} text-white font-medium py-1 rounded-md transition-all duration-200 shadow-sm hover:shadow-md transform hover:-translate-y-0.5 group text-xs h-6`}
        >
