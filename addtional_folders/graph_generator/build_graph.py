...
def visualize_graph(G):
    ...
    for node_type in node_types:
        ...
        node_labels = [f"Node: {node}<br>Type: {node_type}" for node in nodes_of_type]
        ...
        fig.add_trace(go.Scatter(
            ...
            text=[str(node) for node in nodes_of_type],
    ...
    for edge_type in edge_types:
        ...
        for (u, v), edges in edge_groups.items():
            ...
            for i, (key, data) in enumerate(type_edges):
                ...
                edge_hover_text.append(f"From: {u}<br>To: {v}<br>Type: {edge_type}")
    ...
    fig.write_html('dp_interactive.html')
    
    # Optionally display in Jupyter notebook or return the figure
    return fig
...
def get_nodes_info(G):
    ...
    second_node = next(iter(G.nodes(data=True)))[0]
    print("second node:", second_node)

    # Get all outgoing edges from this node
    outgoing_edges = list(G.edges(second_node))
    print("Outgoing edges:", outgoing_edges)

    # Get all incoming edges to this node
    incoming_edges = list(G.in_edges(second_node))
    ...
    # If you want edge data too:
    outgoing_with_data = list(G.edges(second_node, data=True))
    print("Outgoing edges with data:", outgoing_with_data)

def get_leaf_nodes(graph):

    """"
    This function get all the nodes from graph 
    And also if finds the leaf nodes from that graph
    """
    nodes=[]
    nodes_edges = []
    for node, data in graph.nodes(data=True):
        if data['type'] == NODE_TYPE_FILE:
            print(node, data['code'][:100])
            print(data['code'])
        #     # breakpoint()
        # nodes.append(node)
        # nodes_edges.append({"Node":node, "Edges":graph.edges(node, data=True)})

    # print(nodes)
    # print("Nodes and Edges:", nodes_edges)
    # print("Nodes in the graph:", len(nodes))
    print("Edges of this node ",graph.edges("errors.py:DeepgramMicrophoneError", data=True))

    # leaf_nodes = [node for node in graph.nodes() if graph.out_degree(node) == 0]
    # print("Leaf nodes:",len(leaf_nodes))
    return nodes,nodes_edges
    

def leaves_to_root_mapping(G):
    # import networkx as nx
    # import matplotlib.pyplot as plt
    ...
    print(f"Root node: {root_nodes}")
    
    # Visualize the graph with a hierarchical layout
    plt.figure(figsize=(10, 8))
    pos = nx.spring_layout(G)  # You can also try nx.kamada_kawai_layout(G)
    
    # Draw nodes with different colors for roots and leaves
    nx.draw_networkx_nodes(G, pos, nodelist=root_nodes, node_color='green', node_size=500, label='Root')
    nx.draw_networkx_nodes(G, pos, nodelist=leaf_nodes, node_color='red', node_size=500, label='Leaves')
    nx.draw_networkx_nodes(G, pos, nodelist=[n for n in G.nodes() if n not in root_nodes and n not in leaf_nodes],
                        node_color='blue', node_size=500, label='Internal')
    ...
    plt.title("Flow from Root Nodes to Leaf Nodes")
    plt.legend()
    plt.axis('off')
    plt.tight_layout()
    plt.show()
    
    # For topological sort (from root to leaves)
    topo_order = list(nx.topological_sort(G))
    print(f"Topological order (from root to leaves): {topo_order}")
    
    # Find all paths from each root node to each leaf
    for root in root_nodes:
        for leaf in leaf_nodes:
            paths = list(nx.all_simple_paths(G, root, leaf))
            print(f"Paths from root {root} to leaf {leaf}: {paths}")
...
def main():
    ...
    # get_nodes_info(graph)
    # nodes,nodes_to_edges=get_leaf_nodes(graph)
    # print("Nodes in the graph:", len(nodes))
    leaves_to_root_mapping(graph)
    if args.visualize:
