...
6. After `import-export` place call to `code-planner`.
7. After `code-planner` then to `coder_master` who will handle all file generation work.
...
2. Research for each of the topic as identified by the planner.
...
4. After `import-export` call `code-planner`.
5. Next, pass the entire directory structure to `coder_master`
6. `coder_master` will handle the implementation of all files in the structure
7. After `coder_master` completes the code generation, you can call `reporter` to summarize the project
